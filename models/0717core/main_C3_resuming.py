...
def optimize_at_chi(
    ...
    # This dedicated resume driver accepts a checkpoint that has already
    # completed its first Adam phase and entered L-BFGS.  Its state machine is
    # deliberately fixed to exactly: initial L-BFGS → Adam → final L-BFGS.
    _effective_optimizer: str = 'lbfgs'
    _adam_steps_taken: int = 0
    _switched_to_lbfgs: bool = True   # activates the initial L-BFGS guard
    _final_lbfgs_run: bool = False    # initial L-BFGS must hand off to Adam
    ...
    def _start_adam_after_initial_lbfgs(reason: str) -> None:
        """Perform the sole initial-L-BFGS → Adam transition."""
        nonlocal _adam, _lbfgs, _effective_optimizer, _lbfgs_outer_steps
        nonlocal _adam_steps_taken
        print(f"    [resume] L-BFGS→Adam switch at step {step} ({reason})")
        _lbfgs = None
    ...
    while True:
        ...
        if (USE_ADAM_WARMUP_THEN_LBFGS
            ...
            # The resumed run has exactly one Adam phase.  As soon as its
            # existing Δ > -1e-7 trigger fires, enter the terminal L-BFGS.
            if delta > -1e-7:
                _do_switch_to_lbfgs(
                    f"Δ={delta:+.2e} > -1e-7 on Adam step {_adam_steps_taken} "
                    f"(resume schedule → final LBFGS)",
                    final=True)
        # ─────────────────────────────────────────────────────────────────────
        ...
        if _lbfgs_warmed_up and prev_loss is not None and abs(delta) < OPT_CONV_THRESHOLD:
            if _final_lbfgs_run:
                ...
                _start_adam_after_initial_lbfgs(
                    f"initial L-BFGS converged (Δ={delta:.2e})")
            ...
            if _final_lbfgs_run:
                ...
                _start_adam_after_initial_lbfgs(
                    f"initial L-BFGS cycle detected (amplitude={abs(delta):.3e})")
        # ─────────────────────────────────────────────────────────────────────
...
def main():
    ...
    if not args.resume:
        parser.error('main_C3_resuming.py requires --resume CHECKPOINT')
    ...
    print("  Optimizer: resumed L-BFGS → Adam → final L-BFGS "
          "(Adam switches at Δ > -1e-7)")
