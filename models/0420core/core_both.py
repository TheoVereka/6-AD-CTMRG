    ...
    for _restart in range(len(_NOISE_SCALES)):
        ...
        for iteration in range(a_third_max_iterations):
            ...
            if not (lastC21CD is None or lastC21EB is None or lastC21AF is None):
                ...
                #print("SV checks done, not used")

                if _CTM_CONV_MODE == 'SVdifference' and _cv:
                    # Standard mode: SV converged → done.
                    ctm_steps = iteration + 1
                    break

                if _CTM_CONV_MODE == 'both':
                ...
                #print(energy_proxy_fn is not None)
                #print("using conv condition", _CTM_CONV_MODE)

                if energy_proxy_fn is not None and _CTM_CONV_MODE != 'SVdifference':
                    if iteration % _CTM_E_PROXY_INTERVAL == 0:
                        ...
                        if _CTM_CONV_MODE == 'Edifference' and _e_met:
                            ctm_steps = iteration + 1
                            break
                        if _CTM_CONV_MODE == 'both' and _sv_met and _e_met:
                            ctm_steps = iteration + 1
                            break
