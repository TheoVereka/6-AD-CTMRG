...
USE_ADAM_WARMUP_THEN_LBFGS = False
...
def build_vbc_biased_couplings(J1: float, J2: float,
                               branch: str = 'none',
                               orientation: int = 0,
                               field: float = 0.0,
                               rank_order: tuple[int, int, int] = (0, 1, 2),
                               ) -> tuple[list[float], tuple[float, float, float]]:
    """Build the 36 couplings used by the three hexagonal energy contractions.

    The field is trace-free over the three NN bond groups, so their average
    remains J1.  ``plaquette`` makes the two non-oriented groups stronger;
    ``dimer-plaquette`` makes only the oriented group stronger.  Positive h
    therefore seeds the two competing Z6 clock sectors without changing the
    twoC3 variational manifold.  ``rank-split`` assigns (+h, 0, -h) to the
    initial tensor's (rank1, rank2, rank3) correlation groups, reinforcing
    its full three-level ordering.  At h=0 all
    branches are exactly the original isotropic J1-J2 Hamiltonian.

    NN group layout in the three energy functions is
      env1: (G0,G0,G0,G1,G1,G1)
      env2: (G1,G1,G1,G2,G2,G2)
      env3: (G2,G2,G2,G0,G0,G0).
    """
    if branch not in {'none', 'plaquette', 'dimer-plaquette', 'rank-split'}:
        raise ValueError(f"Unknown VBC branch: {branch}")
    if orientation not in (0, 1, 2):
        raise ValueError("VBC orientation must be 0, 1, or 2")
    if field < 0.0:
        raise ValueError("VBC field must be non-negative")
    if sorted(rank_order) != [0, 1, 2]:
        raise ValueError("VBC rank order must be a permutation of 0, 1, 2")
    if branch == 'none' and field != 0.0:
        raise ValueError("A non-zero VBC field requires a seeded VBC branch")

    group_J = [float(J1)] * 3
    if branch == 'plaquette':
        # Stronger AF coupling -> more negative correlation on the other two
        # groups, the ideal plaquette pattern (two strong, one weak).
        group_J[orientation] = J1 - 2.0 * field
        for k in range(3):
            if k != orientation:
                group_J[k] = J1 + field
    elif branch == 'dimer-plaquette':
        # One strong group and two weak groups: the ideal dimer-plaquette sector.
        group_J[orientation] = J1 + 2.0 * field
        for k in range(3):
            if k != orientation:
                group_J[k] = J1 - field
    elif branch == 'rank-split':
        # rank_order=(rank1,rank2,rank3), with rank1 the strongest
        # (most-negative) correlation of the initial tensor.
        rank1_group, rank2_group, rank3_group = rank_order
        group_J[rank1_group] = J1 + field
        group_J[rank2_group] = J1
        group_J[rank3_group] = J1 - field

    if min(group_J) <= 0.0:
        raise ValueError("VBC field is too large: every NN coupling must stay positive")
...
def optimize_at_chi(
    ...
    if OPTIMIZER == 'lbfgs' or USE_ADAM_WARMUP_THEN_LBFGS or skip_adam_warmup:
        ...
        if _effective_optimizer == 'lbfgs' or skip_adam_warmup:
            ...
            if not USE_ADAM_WARMUP_THEN_LBFGS:
                # In pure-LBFGS mode this is already the terminal polishing
                # run.  Mark it explicitly so the post-warmup convergence and
                # cycle guards become active after three outer steps.
                _switched_to_lbfgs = True
                _final_lbfgs_run = True
...
def main():
    ...
    parser.add_argument(
        ...
        default='none',
        help='Trace-free NN pinning sector. At --vbc-field 0 all choices give '
             'the same isotropic physical Hamiltonian.')
    parser.add_argument(
        '--vbc-orientation', type=int, choices=[0, 1, 2], default=0,
        help='NN group singled out by the VBC pinning source.')
    parser.add_argument(
        '--vbc-field', type=float, default=0.0,
        help='Non-negative pinning field h. Use an h-to-zero continuation; '
             'do not compare finite-h branch energies as physical energies.')
    parser.add_argument(
        '--vbc-rank-order', default='0:1:2',
        help='For rank-split: group indices matching the initial tensor '
             'correlations rank1:rank2:rank3 (strongest to weakest).')
    parser.add_argument(
    ...
    parser.add_argument(
        '--resume-tensors-only', action='store_true',
        help='Load tensor values from --resume but ignore its D/chi traversal '
             'position. Use this when re-evaluating the same iPEPS at a lower '
             'or otherwise different environment chi.')
    parser.add_argument(
    ...
    parser.add_argument(
        '--no-mean-field-init', dest='mean_field_init', action='store_false',
        help='Disable mean-field Neel initialisation; needed for VBC branch seeding.')
    parser.add_argument(
    ...
    parser.add_argument(
        '--optimizer', choices=['lbfgs'], default='lbfgs',
        help="Optimiser is deliberately restricted to pure L-BFGS in the "
             "Lyra continuation bundle; Adam is disabled.")
    parser.add_argument(
    ...
    parser.add_argument(
        '--fix-seed', dest='no_seed', action='store_false',
        help='Enable deterministic RNG seeding (counterpart to --no-seed).')
    args = parser.parse_args()
    if args.optimizer != 'lbfgs' or args.adam_warmup_lbfgs:
        parser.error('Adam is disabled in the Lyra bundle; use pure L-BFGS')
    if args.ctm_max_steps < 1:
        parser.error('--ctm-max-steps must be at least 1')
    if args.vbc_field < 0.0:
        parser.error('--vbc-field must be non-negative')
    if args.vbc_field != 0.0 and args.vbc_branch == 'none':
        parser.error('non-zero --vbc-field requires --vbc-branch')
    try:
        vbc_rank_order = tuple(
            int(x) for x in args.vbc_rank_order.replace(',', ':').split(':'))
    except ValueError:
        parser.error('--vbc-rank-order must have the form rank1:rank2:rank3')
    if len(vbc_rank_order) != 3 or sorted(vbc_rank_order) != [0, 1, 2]:
        parser.error('--vbc-rank-order must be a permutation of 0:1:2')
    if args.vbc_field != 0.0 and args.ansatz != 'twoc3':
        parser.error('VBC pinning is intentionally restricted to --ansatz twoc3')
    if args.resume_tensors_only and not args.resume:
        parser.error('--resume-tensors-only requires --resume')
    ...
    print("  Optimizer: pure L-BFGS (Adam disabled by Lyra bundle)")
    ...
    # Build the physical/pinned Hamiltonian before writing hyperparameters.
    # At h=0 this is bit-for-bit the original isotropic coupling layout.
    Js, nn_group_couplings = build_vbc_biased_couplings(
        args.J1, args.J2, args.vbc_branch,
        args.vbc_orientation, args.vbc_field, vbc_rank_order)
    ...
    _hp = dict(
        ...
        resume_tensors_only = args.resume_tensors_only,
        ...
        J1                 = args.J1,
        J2                 = args.J2,
        vbc_branch         = args.vbc_branch,
        vbc_orientation    = args.vbc_orientation,
        vbc_field          = args.vbc_field,
        vbc_rank_order     = list(vbc_rank_order),
        nn_group_couplings = list(nn_group_couplings),
        d_phys             = d_PHYS,
        n_sites            = N_SITES,
    ...
    print(f"  VBC source   : branch={args.vbc_branch}  orientation={args.vbc_orientation}  "
          f"rank1:rank2:rank3={':'.join(map(str, vbc_rank_order))}  "
          f"h={args.vbc_field:g}  (G0,G1,G2)={nn_group_couplings}")
    ...
    if args.resume:
        ...
        print(f"  Resumed from {args.resume}  "
              ...
              f"loss={ckpt_loss:.6f})\n")
        if args.resume_tensors_only:
            print("  [resume-tensors-only] checkpoint tensors loaded; "
                  "D/chi traversal position ignored\n")
            resume_D, resume_chi = None, None
