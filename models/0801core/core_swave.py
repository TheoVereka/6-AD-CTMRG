...
# C6 s-wave single-tensor ansatz

def swave_abcdef_from_a(a_raw: torch.Tensor) -> tuple:
    """Derive the six-site cell using a pure C6 spatial rotation.

    This has exactly the virtual-leg rotation pattern of ``c6ypi`` but does
    not rotate the physical leg. In site order ``(a, b, c, d, e, f)`` the
    result is ``(a, R(a), R²(a), a, R(a), R²(a))``, where
    ``R(a) = a.permute(1, 2, 0, 3)``.
    """
    e = a_raw.permute(1, 2, 0, 3)
    c = a_raw.permute(2, 0, 1, 3)
    return a_raw, e, c, a_raw, e, c


def initialize_swave(D_bond: int, d_PHYS: int,
                     noise_scale: float = 1.0) -> torch.Tensor:
    """Create the free tensor for the C6-rotation-invariant s-wave ansatz."""
    global _USE_FULL_SVD
    a_raw = torch.randn(
        D_bond, D_bond, D_bond, d_PHYS,
        dtype=TENSORDTYPE, device=DEVICE)
