...
MY_OUTPUT_OUTERDIR = '/scratch/izar/chye/0801core'
USE_GPU = True
...
CHI_MAX_LIST  = [ 20, 30, 44, 60, 84,105,112,135,160,176,192]
CHI_STEP_LIST = [  4,  6,  8, 10, 12, 14,  8,  9, 20, 11, 12]
...
# All print() calls — including those from core_colum.py and PyTorch — are captured
...
import core_colum as _core
from core_colum import (
    normalize_tensor,
...
# offload to CPU for cluster GPUs). Hard-coded in core_colum.py; not a tunable.
...
ANSATZ_REGISTRY: dict = {
    ...
    # Two independent columns; no C3 or physical-spin constraint.
    'columnar': {
        'n_params':    2,
        'symmetrize_fn': None,
        'derive_fn':   columnar_abcdef_from_ab,
        'init_fn':     initialize_columnar,
        'ckpt_keys':   ['a_raw', 'b_raw'],
        'yaml_name':   '2tensor_columnar',
        'description': 'Two-tensor columnar ansatz: a=c=e and b=d=f, with no C3 constraint',
    },
}
...
def _run_ctmrg_memory_efficient(
        A, B, C, Dt, E, F,
        chi: int, D_sq: int, max_steps: int, conv_thr: float,
        identity_init: bool, energy_proxy_fn=None) -> tuple:
    """Run D>=8 CUDA CTMRG behind one outer activation checkpoint.

    The core already checkpoints each individual environment update. Without
    this outer boundary, however, every update checkpoint keeps its input
    environment alive until backward. At D=9, chi=126, float64, the six
    transfer tensors of one general (non-C3) environment are about 62 MB;
    three environments over 70 iterations can therefore retain roughly
    13 GB. The outer checkpoint keeps only the six double-layer inputs and the
    final environment during forward, then deterministically recomputes CTMRG
    in backward. It changes no contraction, truncation, or convergence rule.
    """
    def _run(A_, B_, C_, Dt_, E_, F_):
        return CTMRG_from_init_to_stop(
            A_, B_, C_, Dt_, E_, F_, chi, D_sq,
            max_steps, conv_thr, identity_init,
            energy_proxy_fn=energy_proxy_fn)

    use_outer_checkpoint = (
        A.device.type == 'cuda'
        and D_sq >= 64                 # D >= 8; D=9 is the target regime
        and torch.is_grad_enabled()
        and any(t.requires_grad for t in (A, B, C, Dt, E, F))
    )
    if use_outer_checkpoint:
        return _ckpt(_run, A, B, C, Dt, E, F, use_reentrant=False)
    return _run(A, B, C, Dt, E, F)
...
def optimize_at_chi(
    ...
    if ansatz_cfg is None:
        ansatz_cfg = ANSATZ_REGISTRY['columnar']
    ...
    # The three energy expectation functions in core_colum.py write all 36 density
    ...
    def _loss_with_differentiable_ctmrg() -> tuple[torch.Tensor, int]:
        ...
        all28 = _run_ctmrg_memory_efficient(
...
def main():
    ...
    parser.add_argument(
        '--ansatz', choices=list(ANSATZ_REGISTRY.keys()), default='columnar',
            ...
            for chi_idx, chi in enumerate(chis):
                ...
                try:
                    ...
                    def _la_closure():
                        ...
                        all28_la = _run_ctmrg_memory_efficient(
