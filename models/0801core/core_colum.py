...
# Two-tensor columnar ansatz

def columnar_abcdef_from_ab(a_raw: torch.Tensor,
                            b_raw: torch.Tensor) -> tuple:
    """Build a columnar cell with a=c=e and b=d=f.

    No virtual-leg permutation, C3 constraint, or physical-leg rotation is
    applied. The unrestricted CTMRG keeps all nine corners and eighteen
    transfer tensors.
    """
    return a_raw, b_raw, a_raw, b_raw, a_raw, b_raw


def initialize_columnar(D_bond: int, d_PHYS: int,
                        noise_scale: float = 1.0) -> tuple:
    """Create the two independent tensors of the columnar ansatz."""
    global _USE_FULL_SVD
    a_raw = torch.randn(
        D_bond, D_bond, D_bond, d_PHYS,
        dtype=TENSORDTYPE, device=DEVICE)
...
def trunc_rhoCCC(matC21, matC32, matC13, chi, D_squared):
    ...
    if _RECORD_TRUNC_ERROR:
        ...
        U, S, V = truncated_svd_propack(M, chi,
...
def CTMRG_from_init_to_stop(A,B,C,D,E,F,
    ...
    for _restart in range(len(_NOISE_SCALES)):
        ...
        # ── Energy-proxy state (reset each restart so a fresh noisy init
        #    doesn't inherit a stale proxy value from the previous attempt) ──
        _last_e_proxy: float | None = None   # E proxy from previous interval check
        _sv_met: bool = False                 # sticky SV-converged flag for 'both'
        # ── Effective convergence mode ──────────────────────────────────────
        # When no proxy function is provided (energy_proxy_fn is None), fall
        # back to 'SVdifference' regardless of the global _CTM_CONV_MODE.
        # This is critical for clean-evaluation CTMRG: passing proxy=None
        # signals that the caller will compute the full energy AFTER CTMRG
        # converges, so paying the 27-bond-proxy overhead DURING CTMRG is
        # wasteful.  Without this fallback, 'Edifference' or 'both' mode with
        # proxy=None would never converge (proxy branch is skipped, SV branch
        # not entered) and CTMRG would run to max_iterations every time.
        _effective_mode = 'SVdifference' if energy_proxy_fn is None else _CTM_CONV_MODE
        for iteration in range(a_third_max_iterations):

            if not (lastC21CD is None or lastC21EB is None or lastC21AF is None):
                # ── SV check — always computed (collapse detection + SV modes) ──
                ...
                #if _cv: print("SV converged ", end=" ")

                if _effective_mode == 'SVdifference' and _cv:
                    # Standard mode: SV converged → done.
                    ctm_steps = iteration + 1
                    break

                if _effective_mode == 'both':
                    # Latch SV convergence so we don't need it again simultaneously.
                    _sv_met = _sv_met or bool(_cv)

                # ── Energy proxy check ('Edifference' or 'both') ─────────────
                # Only entered when energy_proxy_fn is not None; if proxy=None
                # _effective_mode is already 'SVdifference' so convergence was
                # checked above — this block is unreachable in that case.

                if energy_proxy_fn is not None and _effective_mode != 'SVdifference':

                    # This proxy is convergence-only. An outer checkpoint
                    # reruns it during backward, after the final energy has
                    # populated the observables cache, so proxy RDMs must not
                    # overwrite that final cache.
                    _cache_rhos_before_proxy = _CACHE_RHOS
                    _CACHE_RHOS = False
                    try:
                        ...
                        _CACHE_RHOS = _cache_rhos_before_proxy
