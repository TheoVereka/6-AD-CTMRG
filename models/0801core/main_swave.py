...
# All print() calls — including those from core_swave.py and PyTorch — are captured
...
import core_swave as _core
from core_swave import (
    normalize_tensor,
...
# offload to CPU for cluster GPUs). Hard-coded in core_swave.py; not a tunable.
...
ANSATZ_REGISTRY: dict = {
    ...
    # C6 s-wave single-tensor ansatz: spatial rotation only.
    'swave': {
        'n_params':    1,
        'symmetrize_fn': None,
        'derive_fn':   swave_abcdef_from_a,
        'init_fn':     initialize_swave,
        'ckpt_keys':   ['a_raw'],
        'yaml_name':   '1tensor_C6_swave',
        'description': 'C6-rotation single-tensor s-wave ansatz (no physical-leg rotation)',
    },
    # ── 6-tensor locally-reflected ansatz ────────────────────────────────────
...
C3_COMPATIBLE_ANSATZES = ('neel', 'neel_legacy', 'c6ypi', 'swave', 'c3vypi', 'twoc3')
...
def optimize_at_chi(
    ...
    best_params_tuple contains 1 tensor for single-tensor ansätze (neel/c6ypi/swave/c3vypi),
    ...
    if ansatz_cfg is None:
        ansatz_cfg = ANSATZ_REGISTRY['swave']
    ...
    elif ansatz_cfg in (ANSATZ_REGISTRY['c3vypi'],
                        ANSATZ_REGISTRY['c6ypi'],
                        ANSATZ_REGISTRY['swave']):
    ...
    # The three energy expectation functions in core_swave.py write all 36 density
...
def main():
    ...
    parser.add_argument(
        '--ansatz', choices=list(ANSATZ_REGISTRY.keys()), default='swave',
