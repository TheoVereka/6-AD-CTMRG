...
def discover_new_runs(root: Path, excluded_names: set[str] | None = None) -> ScanResult:
    ...
    for job_dir in sorted(job_dirs):
        ...
        for path in sorted(job_dir.iterdir()):
            ...
            if best_match:
                checkpoints_by_D.setdefault(int(best_match.group("D")), []).append(path)
                continue
        ...
        for D, observations in by_D_observations.items():
            chosen = choose_candidate(observations)
            choice = Choice(
                candidate=chosen,
                origin="new",
    ...
    return ScanResult(completed=completed, incomplete=incomplete, warnings=warnings)
