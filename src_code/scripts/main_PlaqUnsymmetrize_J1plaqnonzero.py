...
DEFAULT_CHI_MAX = {2:999, 3:999, 4: 80, 5:9999, 6:9999, 7:9999, 8:9999, 9:9999, 10:9999, 11:9999}
...
def evaluate_energy_clean(a_raw,
...
def evaluate_observables(a_raw,
...
def save_checkpoint(path: str, a_raw: torch.Tensor, D_bond: int, chi: int,
...
def optimize_at_chi(
    ...
    # initialise tensor — ALWAYS brand-new object, no shared lineage
    ...
    while True:
        ...
        if OPTIMIZER == 'lbfgs':
            ...
            def closure():
                ...
                try:
                    ...
                    if a_raw.grad is not None:
                        a_raw.grad.data = torch.nan_to_num(a_raw.grad.data, nan=0.0, posinf=0.0, neginf=0.0)
                    return loss
                    ...
                    last_ctm_steps = 0
                    if a_raw.grad is None:
                        a_raw.grad = torch.zeros_like(a_raw)
                    else:
                        a_raw.grad.detach_()
                        a_raw.grad.zero_()
                    ...
                    return torch.tensor(1.0e6, dtype=a_raw.real.dtype, device=a_raw.device)
        ...
        if _loss_is_collapsed:
            print(f"    [ZERO-GUARD] loss={loss_item:+.10f} ≈ 0 "
                  f"(env likely near-collapsed); restoring best tensor "
                  f"(best_loss={best_loss:+.10f})")
            with torch.no_grad():
                a_raw.data.copy_(best_a)
...
def main():
    ...
    best_a_by_D: dict[int, torch.Tensor | None] = {D: None for D in D_bond_list}
    global_step = 0
    ...
    if args.resume:
        ...
        best_a_by_D[resume_D] = _new_tensors_from_data(
            (ckpt['a_raw'],))[0]
        del ckpt; gc.collect()
    ...
    for D_bond in D_bond_list:
        ...
        if (best_a_by_D.get(D_bond) is None
                and prev_D is not None
                and best_a_by_D.get(prev_D) is not None):
            print(f"  Warm-starting from D={prev_D} tensor "
                  f"(padding {prev_D}→{D_bond}, noise={args.noise})")
            prev_a = best_a_by_D[prev_D]
            padded_a = pad_tensor(prev_a, prev_D, D_bond, d_PHYS, args.noise)
            best_a_by_D[D_bond] = _new_tensors_from_data((padded_a,))[0]
            del padded_a

        # current best tensor at this D (None = random init at first chi)
        cur_a = best_a_by_D.get(D_bond)
        ...
        for chi_idx, chi in enumerate(chis):
            ...
            best_a, best_loss, global_step = optimize_at_chi(
            ...
            # ── Brand-new tensor from raw data; kill the old run entirely ──
            cur_a = _new_tensors_from_data((best_a,))[0]
            del best_a
            ...
            energy, correlations, magnetizations = evaluate_observables(
                cur_a, Js, SdotS, chi, D_bond, d_PHYS)
            ...
            save_checkpoint(best_path, cur_a, D_bond, chi,
            ...
            if chi_idx < len(chis):
                ...
                with torch.no_grad():
                    energy_la, corr_la, mag_la = evaluate_observables(
                        cur_a, Js, SdotS, chi_la, D_bond, d_PHYS)
                _save_observables_file(
                    os.path.join(output_dir,
        ...
        del cur_a
        # Free tensor from previous D — only needed for warm-start
        # into this D, which has already happened at the top of the D-loop.
        if prev_D is not None and prev_D in best_a_by_D:
            best_a_by_D[prev_D] = None
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
