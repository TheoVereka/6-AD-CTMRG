...
#
# The 6-site honeycomb unit cell (A,B,C,D,E,F) is parametrized by ONE tensor
# ``a_raw`` of shape (D, D, D, d_PHYS).  The 2nd and 3rd virtual legs are
# symmetrized:  a[i,j,k,s] = a[i,k,j,s].
#
# The 6 site tensors are derived by permuting the virtual indices:
#   a = d = a_sym                        (identity)
#   b = e = a_sym.permute(1,2,0,3)       (cyclic: leg0→leg1→leg2→leg0)
#   c = f = a_sym.permute(1,0,2,3)       (swap legs 0,1)
#
# DOF: D * [D*(D+1)/2] * d_PHYS = D²(D+1) for d_PHYS=2.
#
# At the double-layer level:  A=D, B=E, C=F  (3 distinct double-layer tensors).
...
def plaq_abcdef_from_a(a_sym: torch.Tensor) -> tuple:
    ...
    """
    e = a_sym.permute(1, 2, 0, 3)   # C3  rotation
    c = a_sym.permute(2, 0, 1, 3)   # C3² rotation
    #b = a_sym.permute(2, 1, 0, 3)
    #c = a_sym.permute(2, 0, 1, 3)
    #d = a_sym.permute(0, 2, 1, 3)
    #e = a_sym.permute(1, 2, 0, 3)
    #f = a_sym.permute(1, 0, 2, 3)
    ...
    #return (a_sym, b, c, d, e, f)
    #return (a_sym, torch.einsum('ij,...j->...i', U, e), c, torch.einsum('ij,...j->...i', U, a_sym), e, torch.einsum('ij,...j->...i', U, c))
    return (a_sym, e, c, a_sym, e, c)
