...
import numpy as np
import sys
from solver import solve_krylov
...
def main():
    ...
    parser.add_argument('--K', type=int, default=25, help='truncation radius')
    parser.add_argument('--lam', type=float, default=1e-5, help='Tikhonov damping')
    parser.add_argument('--tol', type=float, default=1e-10)
    parser.add_argument('--maxiter', type=int, default=10_000_000)
    parser.add_argument('--lsmr', type=int, default=1)
    args = parser.parse_args()

    kryMethod = 'lsmr' if args.lsmr==1 else 'lsqr'
    ...
    verbose = True,
    save_result = True,
    save_dir  = '/scratch/chye/varPri/'+kryMethod+f'{args.K}',
)

    AE_rel_error = _rel_diff(AE, result.X, full_grid)
    print(f"K=({K1},{K2}), N={N_HILBERT}")
    print(f"    rel ||AE - AE_128|| / ||AE|| = {AE_rel_error:.3e}")

    # 3. Direct Comparisons
    print(f"=== THE VERDICT ===")
    norm_128 = _norm2(result.X, full_grid)
    print(f"||X_float64|| = {norm_128:.6e}")

    # High-frequency analysis
    k_test = (args.K, args.K)
    if k_test in result.X:
        print(f"Deep Dive: Highest Frequency Mode {k_test}")
        print(f"  Float64 Frob Norm: {np.linalg.norm(result.X[k_test],'fro'):.6e}")
