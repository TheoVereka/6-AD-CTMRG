...
def main():
    ...
    parser.add_argument('--K', type=int, default=10, help='truncation radius')
    parser.add_argument('--prec', type=int, default=20, help='mp precision digits')
    parser.add_argument('--lam', type=float, default=1e-8, help='Tikhonov damping')
    ...
    result = solve_eigreg(H, K1, K2, 4, omega, reg=1e-8, verbose=True)

    # The Almost-Invariant Envelope (AE) is natively returned as result.X
    AE_eigreg = result.X
    AE_rel_error = _rel_diff(AE, AE_eigreg, full_grid)
    print(f"K=({K1},{K2}), N={N_HILBERT}")
    print(f"    rel ||AE - AE_eigreg|| / ||AE|| = {AE_rel_error:.3e}")



    print(f"==================================================")
    print(f" THE FINAL LSMR TEST: K={args.K}, damp={args.lam:.0e}")
    print(f"==================================================\n")

    # Assuming you have _make_H and _build_modes in your namespace
    # H, full_grid = _make_H(args.K, args.K, N_HILBERT, 0.0, 3.0, 0.0431, omega)
    # _, _, _, fg_idx = _build_modes(args.K, args.K)
    
    # 1. Float64 LSMR
    t0 = time.time()
    X_128, phi_128, res_128, itn_128, cv_128 = solve_d_lsmr(
        H, args.K, args.K, N_HILBERT, omega, 
        lam=args.lam, tol=args.tol, maxiter=args.maxiter, verbose=True
    )
    t_128 = time.time() - t0

    AE_rel_error = _rel_diff(AE, X_128, full_grid)
    print(f"K=({K1},{K2}), N={N_HILBERT}")
    print(f"    rel ||AE - AE_128|| / ||AE|| = {AE_rel_error:.3e}")

    # 2. Arbitrary Precision LSMR
    t0 = time.time()
    X_ap, phi_ap, res_ap, itn_ap, cv_ap = solve_d_lsmr_arbitraryPrecision(
        H, args.K, args.K, N_HILBERT, omega, 
        lam=args.lam, prec=args.prec, tol=1e-25, maxiter=args.maxiter, verbose=True
    )
    t_ap = time.time() - t0

    # 3. Direct Comparisons
    print(f"=== THE VERDICT ===")
    norm_128 = _norm2(X_128, full_grid)
    norm_ap = _norm2(X_ap, full_grid)
    diff_X = _diff_norm2(X_128, X_ap, full_grid)
    
    print(f"||X_float64|| = {norm_128:.6e}")
    print(f"||X_mpmath||  = {norm_ap:.6e}")
    print(f"Absolute Difference ||X_float64 - X_mpmath|| = {diff_X:.6e}")
    print(f"Relative Error of Float64 = {diff_X/norm_ap:.3e}\n")

    # High-frequency analysis
    k_test = (args.K, args.K)
    if k_test in X_128:
        print(f"Deep Dive: Highest Frequency Mode {k_test}")
        print(f"  Float64 Frob Norm: {np.linalg.norm(X_128[k_test],'fro'):.6e}")
        print(f"  mpmath  Frob Norm: {np.linalg.norm(X_ap[k_test],'fro'):.6e}")
        print(f"  Difference:        {np.linalg.norm(X_128[k_test]-X_ap[k_test],'fro'):.6e}\n")

    print(f"Timing Summary: Float64 took {t_128:.2f}s | mpmath took {t_ap:.2f}s")
    
    AE_rel_error = _rel_diff(AE, X_ap, full_grid)
    print(f"K=({K1},{K2}), N={N_HILBERT}")
    print(f"    rel ||AE - AE_ap|| / ||AE|| = {AE_rel_error:.3e}")
    #sys.stdout.flush()
