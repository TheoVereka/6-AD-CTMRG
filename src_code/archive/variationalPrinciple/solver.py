...
def _build_L_matvec_fast_vectorized(
    ...
    for p_key, H_p in H.items():
        # Compute the explicit d^2 x d^2 Kronecker matrix representation
        C_p = 1j * (np.kron(H_p, I_d) - np.kron(I_d, H_p.T))
        
        # Translate the entire grid simultaneously: k = m + p
        k_coords = modes + np.array(p_key)
        
        # Filter boundaries to keep translations strictly inside the truncated grid
        ...
        # Calculate global matrix block start locations
        row_starts = k_idx * d2
        col_starts = m_idx * d2

        # Rigorous 3D Advanced Indexing: 
        # Broadcasts the d^2 x d^2 entries of C_p across all valid block pairings
        rows = row_starts[:, None, None] + np.arange(d2)[None, :, None]
        cols = col_starts[:, None, None] + np.arange(d2)[None, None, :]
        
        # Direct assignment is perfectly safe and faster because index mappings are unique per p
        L_mat[rows, cols] += C_p[None, :, :]

    # --- 2. Diagonal frequency terms (Fully Vectorized) ---
    freqs = modes @ omega  # shape (M,)
    diag_mask = (freqs != 0.0)
    diag_modes_idx = np.where(diag_mask)[0]
    
    if len(diag_modes_idx) > 0:
        diag_starts = diag_modes_idx * d2
        # Generate sequential diagonal indices for the affected blocks
        diag_rows = (diag_starts[:, None] + np.arange(d2)[None, :]).ravel()
        # Repeat frequencies d^2 times to match flat matrix entries
        diag_vals = 1j * np.repeat(freqs[diag_modes_idx], d2)
...
# ==============================================================================
# Result dataclass (RIGOROUS & SUPER CLEAN)
# ==============================================================================

@dataclass
class SpectralBumpResult:
    """
    Lean container for the exact spectral decomposition.
    Stores ONLY the 7000 modes closest to zero and the pre-projected H coefficients.
    Contains no residual/linear-solver bloat.
    """
    K1: int
    K2: int
    d: int
    omega: np.ndarray
    full_grid_arr: np.ndarray  # Shape (M, 2)
    eigenvalues: np.ndarray    # Shape (7000,) - Purely imaginary, sorted closest to 0
    eigenvectors: np.ndarray   # Shape (N, 7000)
    c_proj: np.ndarray         # Shape (7000,) - Pre-projected <v_i, H>
    reg: float                 # The regularizer window used for the current AE
    X: Optional[Dict] = None   # The composed AE dictionary {k: dxd}

    def __post_init__(self):
        # Automatically compose the AE dictionary upon creation if not provided
        if self.X is None:
    ...
    def composing_filter(self, reg_val: float) -> Dict:
        ...
        Applies YOUR exact smooth bump filter to the pre-calculated 7000-mode subspace.
        """
        # 1. Exact smooth bump filter logic from original code
        x2 = (np.abs(self.eigenvalues) / reg_val)**2
        mask = x2 < 1.0
        print(f"Dense eigendecomposition done.")
        print(f"Modes retained strictly inside bump: {np.sum(mask)} / {len(mask)} (out of {len(mask)} modes with eigenvalue closest to 0)")
        
        f_val = np.zeros_like(self.eigenvalues, dtype=float)
        f_val[mask] = np.exp(1.0 - 1.0 / (1.0 - x2[mask]))

        # 2. Reconstruct X_flat = V @ (f * c_proj)
        c_filtered = f_val * self.c_proj
        X_flat = self.eigenvectors @ c_filtered

        # 3. Pack back into the rigorous {k: dxd matrix} dictionary format
    ...
    def recompose_with_new_reg(self, new_reg: float) -> Dict:
        """Instantly update the AE dictionary under a new bump window size."""
        self.reg = new_reg
        self.X = self.composing_filter(new_reg)
        return self.X

    def save(self, save_dir: Optional[Union[str, Path]] = None, timestamp: Optional[str] = None) -> Path:
        """Saves the minimal spectral backbone to disk using high-speed compression."""
        out_dir = Path(save_dir) if save_dir is not None else Path(r"/scratch/chye/varPri")
        out_dir.mkdir(parents=True, exist_ok=True)
        stamp = timestamp if timestamp is not None else datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    @classmethod
    def load(cls, file_path: Union[str, Path]) -> "SpectralBumpResult":
        """Loads data from disk and auto-reconstructs the AE."""
        with np.load(file_path, allow_pickle=False) as data:
            return cls(
                ...
                d=int(data["d"]),
                omega=np.asarray(data["omega"], dtype=float),
                full_grid_arr=np.asarray(data["full_grid_arr"], dtype=np.int32),
...
# ==============================================================================
# Core solver (UPDATED WITH RIGOROUS SORTING)
# ==============================================================================
...
def solve_eigreg(
    ...
    # 3. RIGOROUS SORTING: Extract the near-null space
    sort_idx = np.argsort(np.abs(evals_hermitian))
    evals_sorted = evals_hermitian[sort_idx]
    V_sorted = V[:, sort_idx]

    # 4. Truncate strictly to the top 7000 modes closest to zero (or N if N < 7000)
    max_modes = min(7000, len(evals_sorted))
    evals_7k = evals_sorted[:max_modes]
    V_7k = V_sorted[:, :max_modes]

    # Recover purely imaginary eigenvalues
    eigenvalues_7k = -1j * evals_7k

    # 5. Pre-compute projections <v_i, H> ONLY for the retained subspace
    if verbose:
        print(f"Projecting H onto the 7000 dominant eigenmodes closest to eigvalue 0...")
    sys.stdout.flush()
    c_7k = V_7k.conj().T @ h_flat

    # 6. Pack into the clean, immutable dataclass
    # The __post_init__ will automatically apply the bump filter and build X
    full_grid_arr = np.asarray(full_grid, dtype=np.int32)
    ...
    if save_result:
        ...
        if verbose:
            print(f"Saved strictly isolated Spectral Result: {out_path}")
    sys.stdout.flush()
