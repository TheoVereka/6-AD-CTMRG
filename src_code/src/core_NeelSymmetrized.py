...
#   sublattice 1 (sites A, C, E): +a
#   sublattice 2 (sites B, D, F): -a
#
...
#
# At the double-layer level, sign cancels: bra⊗ket of (-a) equals bra⊗ket of a,
# so A = B = C = D = E = F.  CTMRG still runs with 6 (identical) double-layer
# tensors — the existing code handles this correctly.
...
def neel_abcdef_from_a(a_sym: torch.Tensor) -> tuple:
    ...
    Néel order on the honeycomb lattice:
        ...
        sublattice 2 (B, D, F): -a_sym

    At the double-layer level the sign cancels, so all 6 double-layer tensors
    are identical.  Gradients flow correctly through the negation.

    Args:
        a_sym: Symmetrized tensor of shape (D, D, D, d_PHYS).

    Returns:
        (a, b, c, d, e, f) where a=c=e=a_sym, b=d=f=-a_sym.
    """
    neg = -a_sym
    return a_sym, neg, a_sym, neg, a_sym, neg
